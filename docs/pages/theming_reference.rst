...
This comprehensive reference documents all style tokens used by questionary prompts, including which tokens each question type uses and how to override them.
...
- Your custom style is merged with ``DEFAULT_STYLE`` using ``merge_styles_default()`` (source: ``questionary/styles.py:9-15``)
...
Defined in ``questionary/constants.py:37-57`` as ``DEFAULT_STYLE``:
...
**Source:** ``questionary/prompts/text.py:85-96``
...
**What renders:** Same as ``text`` but input is masked with ``*`` characters. Internally delegates to ``text(..., is_password=True)`` (source: ``questionary/prompts/password.py:59-61``).
...
**Source:** ``questionary/prompts/confirm.py:71-86``
...
**Source:** ``questionary/prompts/select.py:177-203`` (header), ``questionary/prompts/common.py:397-498`` (list items)
...
**Implementation:** Calls ``select(..., use_shortcuts=True, use_arrow_keys=False)`` (source: ``questionary/prompts/rawselect.py:69-79``)
...
**Source:** ``questionary/prompts/checkbox.py:163-207`` (header), ``questionary/prompts/common.py:397-498`` (list)
...
**Source:** ``questionary/prompts/autocomplete.py:180-206`` (prompt), ``92-99`` (completion items)
...
**Source:** ``questionary/prompts/path.py:192-238``
...
**Source:** ``questionary/prompts/press_any_key_to_continue.py:41-46``
...
**Source:** ``questionary/prompts/common.py:643-670``
